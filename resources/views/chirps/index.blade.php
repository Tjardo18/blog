<x-app-layout>
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('chirps.store') }}">
            @csrf
            <input type="text" name="title" placeholder="{{ __('Title of the message') }}"
            ...
            @foreach ($chirps as $chirp)
                <div class="p-6 flex space-x-2">
                    ...
                    <div class="flex-1">
                        ...
                        <h1 class="mt-4 text-lg text-gray-900">{{ $chirp->title }}</h1>
                        <p class="mt-4 text-md text-gray-900">{{ $chirp->message }}</p>
                        <div class="mt-4">
                    ...
                    <form method="POST" action="{{ route('comments.store') }}">
                        ...
                        <textarea name="content" placeholder="{{ __('Leave a comment...') }}" class="block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">{{ old('content') }}</textarea>
